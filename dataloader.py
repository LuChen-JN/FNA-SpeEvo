...
class NTIDataset(data.Dataset):
    def __init__(self, list_IDs, df,predict=False, max_drug_nodes = 180):
        ...
        for idx in list_IDs:
            ...
            g = self.fc(smiles=smiles, node_featurizer=self.atom_featurizer, edge_featurizer=self.bond_featurizer)
            if g is None:
                print(f"Drop invalid SMILES at index {idx}: {smiles}")
            else:
                valid_ids.append(idx)
        self.list_IDs = valid_ids
        print(f"Kept {len(self.list_IDs)} valid samples out of {len(list_IDs)}")
